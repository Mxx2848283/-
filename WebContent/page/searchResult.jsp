	...
	<TBODY>
		<TR>
			<TD class=left2 background=${ctx}/images/page/type/right5_1_2.jpg
				height=30><SPAN id=LblType>搜索结果</SPAN></TD>
		</TR>
		<TR valign=top>
			<TD class="left,main" background=${ctx}/images/page/type/right5_2.jpg
				height=255>
				<TABLE id=TypeGrid style="WIDTH: 550px; BORDER-COLLAPSE: collapse"
					...
					<TBODY>
						...
						<c:forEach items="${searchMessageList }" var="message"
							...
							<TR align=middle>
								<TD><c:if test="${message.flag == 0 }">[出售]</c:if> <c:if
										test="${message.flag == 1 }">[求购]</c:if></TD>
								<TD style="WIDTH: 40%"><A id=TypeGrid_ctl12_Hyperlink2
								...
								<TD align=middle><FONT style="FONT-SIZE: 10pt" face=宋体>合计
										...
										<c:if test="${cur_page > 0 }">
											<A id=pageHead
												...
												href="searchMessagePage.do?cur_page=0&action=page"> 首页</A>
